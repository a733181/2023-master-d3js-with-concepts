...
function linearScaleHandler(d_start, d_end, r_start, r_end) {
  const LINEAR_SCALE = d3
    .scaleLinear()
...
function axisHandler(type, ticks, scale) {
  let AXIS;
  if (type == 'left') {
...
// axis group creation function
function groupHandler(context, id, x, y) {
  const GROUP = context
    .append('g')
    .attr('id', id)
...
*/
function minOrMaxHandler(obj, status, r_status) {
  let value, valueKey;
  for (key in obj) {
...
for (let i = 0; i < SALES_DATA.length; i++) {
  max_value.push(minOrMaxHandler(SALES_DATA[i], 'max', 0));
}
// min values
let min_value = [];
for (let i = 0; i < SALES_DATA.length; i++) {
  min_value.push(minOrMaxHandler(SALES_DATA[i], 'min', 0));
}
// y axis
const STAT_Y_AXIS_SCALE = linearScaleHandler(
  0,
  DATA_LENGTH - 1,
  BUFFER,
  SVG_H_R1 - BUFFER
);
const STAT_Y_AXIS = axisHandler('left', 0, STAT_Y_AXIS_SCALE);
const STAT_Y_AXIS_G = groupHandler(STAT_SVG, 'statYAxis', BUFFER, 0);
STAT_Y_AXIS(STAT_Y_AXIS_G);
// x axis
const STAT_X_AXIS_SCALE = linearScaleHandler(
  0,
  d3.max(max_value),
  BUFFER,
  SVG_W_R1 - BUFFER
);
const STAT_X_AXIS = axisHandler('bottom', 4, STAT_X_AXIS_SCALE);
const STAT_X_AXIS_G = groupHandler(STAT_SVG, 'statXAxis', 0, SVG_H_R1 - BUFFER);
STAT_X_AXIS(STAT_X_AXIS_G);
// create a group for each data point
SALES_DATA.forEach((obj, index) =>
  STAT_SVG.append('g').attr('id', `stat${index}`)
);
// render bars
...
SALES_DATA.forEach((obj, index) => {
  d3.select(`#stat${index}`)
    .selectAll('rect')
    .data([
      minOrMaxHandler(SALES_DATA[index], 'min', 0),
      minOrMaxHandler(SALES_DATA[index], 'max', 0),
    ])
    .join('rect')
    ...
    .style('fill', (d, i) =>
      i == 0
        ? CAT_COLOR_SCALE(minOrMaxHandler(SALES_DATA[index], 'min', 1))
        : CAT_COLOR_SCALE(minOrMaxHandler(SALES_DATA[index], 'max', 1))
    )
    .attr('rx', '3')
    .attr('ry', '3');
});
...
// Iterations
function meanAndMapHandler(obj) {
  let value = [],
    newObj = {},
    i = 0;
  for (key in obj) {
...
// y axis
const ITER_Y_AXIS_G = groupHandler(ITER_SVG, 'iterYAxis', BUFFER, 0);
STAT_Y_AXIS(ITER_Y_AXIS_G);
// x axis
const ITER_X_AXIS_G = groupHandler(ITER_SVG, 'iterXAxis', 0, SVG_H_R1 - BUFFER);
STAT_X_AXIS(ITER_X_AXIS_G);
// create a group for each data point
SALES_DATA.forEach((obj, index) =>
  ITER_SVG.append('g').attr('id', `iter${index}`)
);
SALES_DATA.forEach((obj, index) => {
  d3.select(`#iter${index}`)
    .selectAll('rect')
    .data(meanAndMapHandler(obj))
    .join('rect')
    ...
    .style('fill', (d, i) =>
      CAT_COLOR_SCALE(meanAndMapHandler(SALES_DATA[index])[i][0])
    )
    .attr('rx', '3')
    .attr('ry', '3');
});
...
// y axis
const SEAR_Y_AXIS_SCALE = linearScaleHandler(
  d3.max(max_value),
  0,
  BUFFER,
  SVG_H_R2 - BUFFER
);
const SEAR_Y_AXIS = axisHandler('left', 4, SEAR_Y_AXIS_SCALE);
SEAR_Y_AXIS.ticks(4, '~s').tickPadding(0).tickSize(1);
const SEAR_Y_AXIS_G = groupHandler(SEAR_SVG, 'searYAxis', BUFFER, 0);
SEAR_Y_AXIS(SEAR_Y_AXIS_G);
// x axis
const SEAR_X_AXIS_SCALE = linearScaleHandler(
  0,
  entries.length - 1,
  BUFFER,
  SVG_W_R2 - BUFFER
);
const SEAR_X_AXIS = axisHandler('bottom', 0, SEAR_X_AXIS_SCALE);
const SEAR_X_AXIS_G = groupHandler(SEAR_SVG, 'searXAxis', 0, SVG_W_R2 - BUFFER);
...
const TRAN_Y_AXIS_SCALE = linearScaleHandler(sum, 0, BUFFER, SVG_H_R2 - BUFFER);
const TRAN_Y_AXIS = axisHandler('left', 4, TRAN_Y_AXIS_SCALE);
TRAN_Y_AXIS.ticks(4, '~s').tickPadding(0).tickSize(1);
const TRAN_Y_AXIS_G = groupHandler(TRAN_SVG, 'tranYAxis', BUFFER, 0);
TRAN_Y_AXIS(TRAN_Y_AXIS_G);
//
const groupedWidth = (SVG_W_R2 - 2 * BUFFER) / grouped.length;
// x axis
const TRAN_X_AXIS_SCALE = linearScaleHandler(
  0,
  grouped.length - 1,
  BUFFER,
  SVG_W_R2 - BUFFER
);
const TRAN_X_AXIS = axisHandler('bottom', 0, TRAN_X_AXIS_SCALE);
const TRAN_X_AXIS_G = groupHandler(TRAN_SVG, 'tranXAxis', 0, SVG_W_R2 - BUFFER);
SEAR_X_AXIS(TRAN_X_AXIS_G);
//
function groupedHandler(region) {
  let eachRegion = [];
  ...
  .attr(
    ...
    (d, i) =>
      linearScaleHandler(
        ...
        grouped.length - 1,
        BUFFER,
        SVG_W_R2 - BUFFER - groupedWidth
      )(i) + 5
  )
  .attr('y', (d, i) =>
    linearScaleHandler(sum, 0, BUFFER, SVG_H_R2 - 2 * BUFFER)(groupedHandler(d))
  )
...
//
const SETS_Y_AXIS_SCALE = linearScaleHandler(
  Array.from(commonValue),
  0,
  BUFFER,
  SVG_H_R2 - BUFFER
);
const SETS_Y_AXIS = axisHandler('left', 5, SETS_Y_AXIS_SCALE);
SETS_Y_AXIS.ticks(5, '~s').tickPadding(0).tickSize(1);
const SETS_Y_AXIS_G = groupHandler(SETS_SVG, 'setsYAxis', BUFFER, 0);
SETS_Y_AXIS(SETS_Y_AXIS_G);
//
// x axis
const SETS_X_AXIS_SCALE = linearScaleHandler(
  0,
  commonArray.length - 1,
  BUFFER,
  SVG_W_R2 - BUFFER
);
const SETS_X_AXIS = axisHandler('bottom', 0, SETS_X_AXIS_SCALE);
const SETS_X_AXIS_G = groupHandler(SETS_SVG, 'setsXAxis', 0, SVG_W_R2 - BUFFER);
...
//
SETS_SVG.selectAll('rect').each((d, i, n) => {
  d3.select(n[i])
    .transition()
    .delay(200 * (i + 1))
    .duration(1500)
