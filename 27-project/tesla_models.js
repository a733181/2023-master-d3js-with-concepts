...
RAW_DATA.then((data) => {
  ...
  const COLOR = d3.interpolate('#00ff00', '#ff0000');
  ...
  document.querySelector('#mButtons').addEventListener('click', (e) => {
    let TAGET_ID;
    if (e.target.localName === 'button') {
      TAGET_ID = e.target.id;
    }

    for (car of DATA) {
      if (car.id === TAGET_ID) {
        displayInfoHandler(car, DATA.indexOf(car));
      }
    }
  });
  const HANDLER_CALL = [
  ...
  function displayInfoHandler(car, colorIndex) {
    document.querySelector('#mInfo').innerHTML = '';

    COLUMNS.forEach((col, index) => {
      document.querySelector('#mInfo').innerHTML += `<p>${col}:${HANDLER_CALL[
        index
      ](Object.values(car)[index])}</p>`;
    });
    d3.selectAll('#mInfo p').style(
      'color',
      COLOR((colorIndex + 1) / DATA.length)
    );
