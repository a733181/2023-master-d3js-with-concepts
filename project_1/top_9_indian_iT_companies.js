...
list.on('click', (event) => {
  indian9IT.forEach((company) => {
    if (event.target.className == company.code) {
      ...
      d3
        .select('#revenue')
        .node().innerHTML = `Revenue (INR Mn): <span>${company.revenue.toLocaleString()}</span>`;
      d3
        .select('#marketcap')
        .node().innerHTML = `Market Cap (INR Mn): <span>${company.marketCap.toLocaleString()}</span>`;
      d3
        .select('#employees')
        .node().innerHTML = `Employees: <span>${company.employees.toLocaleString()}</span>`;
      d3
        .select('#salesgrowth')
        .node().innerHTML = `Sales Growth (3-year): <span>${company.salesGrowth}%</span>`;
...
function resetTitle(seln) {
  d3.select(seln).select('.charttitle').html(null);
}

function resetChart(seln) {
  d3.select(seln).select('svg').selectAll('rect').remove();
}

function resetChartText(seln) {
  d3.select(seln).select('svg').selectAll('text').remove();
}

function setTitle(seln, title) {
  d3.select(seln).select('.charttitle').text(title);
}
  ...
  d3.select(seln)
    ...
    .each((d, i, n) => {
      ...
      elem
        ...
        .attr('y', '0')
        .text(
          (d) => d.code.toUpperCase() + ' | ' + d[parameter].toLocaleString()
        )
