...
const densityList = [];
top10PopDensity.forEach((item) => densityList.push(item.density));
...
document.querySelector('#scalesubmit').addEventListener('click', (e) => {
  const INPUT_VALUE = document.querySelector('#scaleinput').value;

  if (INPUT_VALUE >= 0 && INPUT_VALUE <= 100) {
    document.querySelector('#scaleoutput').innerHTML = POP_DEN_LIN_SCALE.invert(
      (INPUT_VALUE / 100) * SVG_WIDTH
    ).toFixed(2);
  }
});
