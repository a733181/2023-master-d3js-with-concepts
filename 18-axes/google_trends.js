const D3_JS_DATA = [64, 71, 21, 62, 62, 83, 41, 40, 60, 60, 49, 38, 84];
...
// -----
// RENDER BARS
let maxBarWidth = X_AXIS_SCALE(END_DATE) - X_AXIS_SCALE(new Date(2020, 2, 29));
maxBarWidth = Math.floor(maxBarWidth) - 2;

D3_JS_DATA.forEach((count) =>
  SVG.append('g').attr('class', 'pair').data([count])
);
...
document.querySelector(
  '#note p'
).innerHTML = `Note: Y-Axis is the count | X-Axis is the week | Average search count for the period was: <b>${Math.round(
  d3.mean(D3_JS_DATA)
)}</b>`;
